...
contract Turing is ERC20{
    ...
    modifier LimitTuring(uint256 Turings) {
        require((Turings > 0) && (Turings <= 2*10**18), "Input Error - Value not accepted");
        _;
    }

    modifier isValidReceiver(string memory nickname) {
        require(sys.usr[nickname].valid == true, "Receiver Error - Null receiver");
        _;
    }
    ...
    */
    ...
    User lastSender;
    uint256 lastAmount;
    VoteTransaction public lastVoted;
    function vote(string memory nickname, uint256 Turings) public isAuthorizedUser isValidReceiver(nickname) VoteOthers(nickname) LimitTuring(Turings) VoteOnlyOnce(nickname) isEndedVote{
        _mint(sys.usr[nickname].addr, Turings);
        _mint(sys.addr[msg.sender].addr, 2*10**17);
        sys.voted[sys.addr[msg.sender].name][sys.usr[nickname].name] = true;
        lastReceiver = sys.usr[nickname];
        lastSender = sys.addr[msg.sender];
        lastAmount = Turings;
        lastVoted = VoteTransaction({receiver: lastReceiver, sender: lastSender, amount: lastAmount});
    }
    ...
    */
    ...
    function whoItIs() public view returns(User memory){
        return sys.addr[msg.sender];
    }
